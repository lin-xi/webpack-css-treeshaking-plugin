...
function selectorSort(selector) {
...
module.exports = postcss.plugin('list-selectors', function (options, callback) {
  ...
  return function (cssRoot, postcssResult) {
    ...
    var sortedUniqueSelectors = _.chain(accumulatedSelectors).uniq().sortBy(selectorSort).value();
    ...
    _.forOwn(selectorObj.simpleSelectors, function (selectorList, key) {
      selectorObj.simpleSelectors[key] = _.chain(selectorList).flatten().uniq().sortBy(selectorSort).value();
    });
    ...
    var classesNotInJS = selectorObj.filter(function (className) {
    ...
    cssRoot.walkRules(function (rule) {
      console.log(rule);
      // Ignore keyframes, which can log e.g. 10%, 20% as selectors
      if (rule.type === 'class' && classesNotInJS.indexof(rule.name) !== -1) {
        rule.remove();
      }
    });
