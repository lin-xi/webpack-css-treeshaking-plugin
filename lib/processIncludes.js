...
var TOP_LEVEL_INCLUDES = ['selectors', 'simpleSelectors', 'simple'];
var SIMPLE_INCLUDES = ['classes', 'ids', 'attributes', 'types'];
...
module.exports = function (selectorList, includes, postcssResult) {
  ...
  return _.reduce(includes, function (r, incl) {
    ...
    postcssResult.warn('Invalid include "' + incl + '" passed. ' + 'The possibilities are: ' + TOP_LEVEL_INCLUDES.concat(SIMPLE_INCLUDES).join(', ') + '. ' + 'You\'ll get the full selector list now.');
    return selectorList;
  }, {});
};